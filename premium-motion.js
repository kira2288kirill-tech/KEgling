...
(function () {
    ...
    const appPerformanceLite = document.documentElement.classList.contains("app-performance-lite");
    const reduceMotion = () =>
        appPerformanceLite || (window.matchMedia && window.matchMedia("(prefers-reduced-motion: reduce)").matches);
    const isCoarse = () =>
        window.matchMedia && window.matchMedia("(hover: none), (pointer: coarse)").matches;
    const allowParallax = () =>
        !appPerformanceLite && !reduceMotion() && window.matchMedia && window.matchMedia("(min-width: 768px)").matches;
